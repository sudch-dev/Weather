...
def pick_location_from_query() -> Tuple[float, float, str, str]:
    """Priority: ?lat&lon&name&region  → ?city=kolkata|durgapur → default Durgapur."""
...
def assemble_payload(lat: float, lon: float) -> Dict[str, Any]:
    ...
    # Meta
    gen_ms = _safe_float(forecast.get("generationtime_ms"), 0.0)
    meta = {
    ...
    # Hourly (use first 24 for table + compute today's precip probability)
    ...
    # Today's precip prob = nearest hour from now if available, else first item
    today_precip_prob = None
    try:
        # find index closest to now (UTC string list)
        if times_h and "precipitation_probability" in hourly:
            now_utc = datetime.utcnow()
            best_i, best_diff = 0, 10**9
            for idx, ts in enumerate(times_h):
                try:
                    dt = datetime.strptime(ts, "%Y-%m-%dT%H:%M")
                    diff = abs((dt - now_utc).total_seconds())
                    if diff < best_diff:
                        best_diff, best_i = diff, idx
                except Exception:
                    break
            today_precip_prob = hourly["precipitation_probability"][best_i]
    except Exception:
        today_precip_prob = None
    ...
    # Alerts for "Today" strip
    temp_now = current.get("temperature")
    precip_alert = (today_precip_prob is not None) and (today_precip_prob >= 85)
